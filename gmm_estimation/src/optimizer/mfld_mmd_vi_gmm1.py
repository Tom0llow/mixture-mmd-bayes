...
from core.simulators import simulate_gaussian_mixture_location


def mfld_mmd_vi_gmm1(
    X: torch.Tensor,
    beta: float,
    steps: int = 1000,
    ...
    * (0.1 + 0.9 * 0.5 * (1.0 + math.cos(math.pi * (t - 1) / max(1.0, T - 1)))),
    ...
    simulate_fn: Callable[..., torch.Tensor] = simulate_gaussian_mixture_location,
    sigma_x2: float = 1.0,
    ...
    """GMM generator variant where theta is (M, C, d) component means.

    Returns:
        m_hat: (C, d) mean of theta over M (CPU tensor)
        sigma2_hat: (C, d) variance over M (CPU tensor)
        theta_particles: (M*C, d) flattened particles if return_particles else None
    """
    ...
    theta = (prior_tau2**0.5) * torch.randn(M, C, d, device=device, dtype=dtype)
    ...
    for t in range(1, steps + 1):
        ...
        eps = torch.randn(M, S_total, d, device=device, dtype=dtype)
        Y = simulate_fn(theta, eps_y=eps, sigma_x2=sigma_x2).reshape(M * S_total, d)

        mmd_term = E_mmd2(X, Y, kernel_fn, bw, M=M, S=S_total)
        prior_term = (lambda_kl / (2.0 * prior_tau2)) * theta.pow(2).sum() / (M * C)
    ...
    theta_particles = theta.detach().reshape(M * C, d) if return_particles else None
